...
static void __k_dev_fb_scroll(){
    ...
    if(info.type == 2){
        if(pos_y + 1 < info.height){
            ...
            memmove(framebuffer, framebuffer + info.pitch, info.pitch * info.height);
        }
    }else{
        PSF_font *font = (PSF_font*)&_binary_font_psf_start;
        pos_y += font->height;
...
static void __k_dev_fb_draw_char(uint16_t c, uint32_t cx, uint32_t cy, uint32_t fg, uint32_t bg){
    PSF_font *font = (PSF_font*)&_binary_font_psf_start;
