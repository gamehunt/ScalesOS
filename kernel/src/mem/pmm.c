...
uint32_t  first_free_index = 0xFFFFFFFF;
...
void     k_mem_pmm_init(multiboot_info_t* mb){
    ...
    for(uint32_t i = 0; i < mb->mmap_length; i += sizeof(multiboot_memory_map_t)){
            multiboot_memory_map_t* mmap = (multiboot_memory_map_t*)(mb->mmap_addr + 0xC0000000 + i);
            if(mmap->type != MULTIBOOT_MEMORY_AVAILABLE){
                k_info("   0x%.9llx - 0x%.9llx Type: 0x%x", mmap->addr, mmap->addr + mmap->len, mmap->type);
            }else{
                k_info("   0x%.9llx - 0x%.9llx Type: 0x%x <-- Usable", mmap->addr, mmap->addr + mmap->len, mmap->type);
    ...
    k_debug("Final bitmap size: %d bytes", k_mem_pmm_bitmap_size());
}
...
pmm_frame_t k_mem_pmm_alloc_frame(){
    ...
    pmm_frame_t frame = 0;
    for(int i = 0; i < 8; i++){
        if(bitmap[first_free_index] & (1 << i)){
            bitmap[first_free_index] &= ~(1 << i);
            frame = FRAME(first_free_index, i);
            break;
        }
    }
